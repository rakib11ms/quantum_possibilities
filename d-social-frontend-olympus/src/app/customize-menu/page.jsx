...
import CustomLeftbar from "@/component/CustomizeAllMenu/CustomLeftSortable";
import CustomRight from "@/component/CustomizeAllMenu/CustomRightSortable";
import LeftSidebarCusomizable from "@/component/CustomizeAllMenu/LeftSidebarCusomizable";
...
import RightSidebarCusomizable from "@/component/CustomizeAllMenu/RightSidebarCusomizable";
import { navContainer, rightSidebarContainer } from "@/utils/nav";
...
const Page = () => {
  ...
  // State variables for managing draggable items
  ...
  const handleFollowPage = (page_id, index) => {
    // Your logic for handling follow page action
  };

  // Function to handle the end of the drag operation
  const handleDragEnd = (result) => {
    // Check if a valid destination is available
  ...
  const handleAvatarClick = () => {
    console.log("AAAAAA", menuVisible);
    setMenuVisible(!menuVisible);
  };
  console.log("BBB", menuVisible);
  ...
  const handleAvatarClickNoti = () => {
    console.log("AAAAAA", menuVisibleNoti);
  ...
  console.log("BBB", menuVisibleNoti);
  const handleClickOutsideNoti = (event) => {
    if (menuRefNoti.current && !menuRefNoti.current.contains(event.target)) {
      setMenuVisibleNoti(false);
    }
  };
  //   const [menuVisibleChat, setMenuVisibleChat] = useState(false);
  //   const menuRefChat = useRef(null);

  //   const handleAvatarClickChat = () => {
  //     console.log("AAAAAA", menuVisibleChat);
  //     setMenuVisibleChat(!menuVisibleChat);
  //   };
  // console.log("BBB", menuVisibleChat);
  //   const handleClickOutsideChat = (event) => {
  //     if (menuRefChat.current && !menuRefChat.current.contains(event.target)) {
  //       setMenuVisibleNoti(false);
  //     }
  //   };
  ...
  // console.log("message noti", messageNotifications)  
  const [sortableList, setSortableList] = useState(navContainer);

  const [mainItems, setMainItems] = useState([]);

  const [sortableListRight, setSortableListRight] = useState(rightSidebarContainer);
  ...
  useEffect(() => {
    axiosInstance
      .get(`/api/get-all-private-one-to-one-message-notifications`)
      .then((res) => {
    ...
    axiosInstance
      ...
      .then((res) => {
        if (res.data.status == 200) {
          ...
          const leftMenuData = res.data.menu.find(i => i.type == 'leftMenu')?.content || navContainer
          const rightMenuData = res.data.menu.find(i => i.type == 'rightMenu')?.content || rightSidebarContainer

          setSortableList(navContainer.filter(i => {
            if (leftMenuData.find(j => j.id === i.id)) return false;
            else return true;
          }))

          setSortableListRight(rightSidebarContainer.filter(i => {
            if (rightMenuData.find(j => j.id === i.id)) return false;
            else return true;
          }))

          const conLeftMenu = [], conRightMenu = []
          for (const i of leftMenuData) {
            const temp = navContainer.find(j => j.id === i.id)
            if (temp) {
              conLeftMenu.push(temp)
            }
          }
          setMainItems(conLeftMenu)
    ...
    <DragDropContext onDragEnd={handleDragEnd}>
      <div>
        ...
        >
          ...
          {/* middle  */}
          <div >
            <div className='customize-navbar-div'>
              <div className='row'>
                  ...
                  <div className='nav-right-head-icons-div'>
                    <span
                      ...
                      onClick={() => {
                        ...
                        axiosInstance
                          .get(
                            `/api/delete-all-private-one-to-one-message-notifications`
                          )
                          .then((res) => {
                            if (res.data.status == 200) {
                          ...
                          <div
                            ...
                            {filteredUsers?.map((item, i) => {
                              return (
                                <>
                                  <div
                                    class='last-message-single-item d-flex justify-content-between pb-3 '
                                    onClick={() => {
                      ...
                      <div className='menuNoti' ref={menuRefNoti}>
                        ...
                        <div>
                          <div className='Notification-headertag-div'>
                            {" "}
                            <h6 className='all-notify-text'>
                              Notifications
                            </h6>
                            <Link href='#' className='link-mark'>
                              {" "}
                              <span className='link-mark-as'>
                                Mark all as read
                              </span>
                              <div className=''></div>
                            </Link>{" "}
                          ...
                          <div>
                            <ul className='nav-dash-sidebar-div'>
                              <li
                                ...
                                className={`nav-li-sec ${activeDiv === 1
                                  ? "nav-dash-sidebar"
                                  : ""
                                  }`}>
                                <div>
                              ...
                              <li
                                ...
                                className={`nav-li-sec ${activeDiv === 2
                                  ? "nav-dash-sidebar"
                                  : ""
                                  }`}>
                                <div>Unread</div>
                        ...
                        <div>
                          ...
                          <div className='nav-auth-dropdown-li-div'>
                            <ul>
                              ...
                              <li
                                className='nav-auth-dropdown-li'
            ...
            <Grid sx={{
              ...
              <div style={{
                ...
                <img
                  src={'/custom-svg-icon/not_Customizable.jpg'}
                ...
                }}>“Not Customizable”</div>

              </div>

              <div >
                <div className='leftSidebarCustom-div'>
                  <RightSidebarCusomizable
                    mainItemsRight={mainItemsRight}
                    setSortableListRight={setSortableListRight}
                    setMainItemsRight={setMainItemsRight}
          ...
          <div>
            <CustomRight
              mainItems={mainItems}
              mainItemsRight={mainItemsRight}
              sortableListRight={sortableListRight}
              setSortableListRight={setSortableListRight}
              setMainItemsRight={setMainItemsRight}
            />
          </div>
...
// import CustomLeftbar from "../../component/CustomizeAllMenu/CustomLeftbar"

// export default function Home() {
//   return (
//     <div>
//       <Head>
//         <title>React Beautiful DnD Example</title>
