...
function EducationItem({ setAllEducation, getAllEducation, education, editOption }) {
    ...
    const handleDelete = (_id) => {
        axiosInstance.post(`${host}/api/delete-education/${_id}`).then((res) => {
            if (res.data.status == 200) {
                setAllEducation(state => state.filter(item => item._id !== _id))
                ...
                <div className='prfworkabt-full-img-div'>
                    <div className='prfworkabt-img-div'>
                        <div className='tas_letter_container'>
                            <p className='tas_letter'>{education?.institute_name.slice(0, 1)}</p>
                        </div>
                    </div>
                    <div>
                        ...
                        <span className='wr-stasn-texts'>
                            {getMonthYear(education.startDate)} - {education.is_Stuyding ? "Current" : getMonthYear(education.endDate)}
                        </span>
                    </div>
                </div>
            </div>
            {editOption ? <div className='col-2 col-sm-2 col-md-2 col-lg-2 col-xl-2'>
                <div className='d-flex tas_privacy_Edit_container'>
                    <span className='tas_privacy_container'>
                        {education?.privacy === "public" ? <PublicSmallIcon /> : education?.privacy === "friends" ? <FriendSmallIcon /> : <OnlymeSmallIcon />}
                    </span>
                    <span
                        ...
                        <li>
                            <div className='share-popup'>
                                ...
                                <Modal isOpen={isOpenModal.education} onRequestClose={() => setIsOpenModal({ education: false })} style={customStyles}>
                                    {isOpenModal.education && <EducationModal getAllEducation={getAllEducation} setIsOpenModal={setIsOpenModal} education={education} />}
                                </Modal>
                            </div>
                        </li>
                        <li>
                            <div className='share-popup'>
                                ...
                                <span onClick={() => handleDelete(education._id)} className='share-text-wr-dlt tas_workplace_delete_btn'>
                                    Delete
                                </span>
                            </div>
                        </li>
