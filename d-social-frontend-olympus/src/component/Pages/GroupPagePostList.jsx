...
import PostWrapper from "../Group/GroupPost/PostWrapper";
...
const GroupPagePostList = ({ pageDetails }) => {
  ...
  async function allPostss() {
    ...
    const formData = {
      group_id: pageDetails._id,
    };

    axiosInstance.post("/api/get-group-posts", formData)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    if (pageDetails && pageDetails._id && isApiSubscribed) {
  ...
  const shareToPublic = (post_id) => {
    ...
    axiosInstance.post('/api/save-share-group-post', {
      ...
      axiosInstance
        ...
        .then((res) => {
          if (res.data.status === 200) {
            ...
            axiosInstance
              .get(`/api/view-single-main-group-post-with-comments/${postSubmitID}`)
              .then((res) => {
                if (res.data.status === 200) {
                  setCommentTextState({})
                  // setCommentImageOrVideoFile(null);
      ...
      axiosInstance
        ...
        .then((res) => {
          if (res.data.status == 200) {
            ...
            axiosInstance
              .get(
                `/api/view-single-main-group-post-with-comments/${commentReplyPostId}`
              )
              .then((res) => {
                if (res.data.status === 200) {
                  setCommentTextState("");
  ...
  useEffect(() => {
    if (reactionPostId || postReactionCurrentType) {
      ...
      axiosInstance
        .post("/api/save-reaction-group-main-post", submitReactionData)
        .then((res) => {
          if (res.data.status == 200) {
  ...
  useEffect(() => {
    if (clicktoSeeWhoReactedPostId) {
      axiosInstance
        .get(
          `/api/reaction-user-lists-of-direct-group-post/${clicktoSeeWhoReactedPostId}`
        )
        .then((res) => {
          if (res.data.status == 200) {
  ...
  console.log('edit reply comment fetch Data', "", editMainCommentPostId, editMainCommentReplyId, editReplyCommentData, editReplyCommentImageOrVideo)

  async function getEditMainCommentData() {
    axiosInstance.get(`/api/edit-comment-by-direct-group-post/${editMainCommentPostId}/${editMainCommentId}/main_comment`).then((res) => {
      if (res.data.status == 200) {
  ...
  async function getEditMainCommentReplyData() {
    axiosInstance.get(`/api/edit-comment-by-direct-group-post/${editMainCommentPostId}/${editMainCommentReplyId}/reply_comment`).then((res) => {
      if (res.data.status == 200) {
  ...
  const handleCommentReactionSubmit = (e, commentReactionPostID, commentReactionCommentID, commentRepliesID, commentReactionCurrentType) => {
    e.preventDefault();
    ...
    axiosInstance.post("api/save-comment-reaction-of-direct-group-post", commentReactionData)
      .then((res) => {
        if (res.data.status == 200) {
          setRenderComments(res.data);
          toast.success(res.data.message, {
          ...
          axiosInstance
            .get(
              `/api/view-single-main-group-post-with-comments/${commentReactionPostID}`
            )
            .then((res) => {
              if (res.data.status === 200) {
                ...
                const postIndex = allPosts.findIndex(
                  (post) => post._id === commentReactionPostID
                );
  ...
  const [reactedUserListsOfDirectPostsComments, setReactedUserListsOfDirectPostsComments] =
    useState([]);
  ...
  function openWhoesReactOnCommentModal(e, reactionPostId, commentId, commentRepliesId) {
    ...
    // setClicktoSeeWhoReactedPostId(reactionPostId);
    axiosInstance.get(`/api/reaction-user-lists-comments-of-a-direct-group-post/${reactionPostId}/${commentId}/${commentRepliesId || "null"}`).then((res) => {
      if (res.data.status == 200) {
    ...
    axiosInstance
      ...
      .then((res) => {
        if (res.data.status == 200) {
          ...
          axiosInstance
            .get(`/api/view-single-main-group-post-with-comments/${postid}`)
            .then((res) => {
              if (res.data.status === 200) {
                setpostid(null);
                setcommentid(null);
  ...
  return (
    <div className='ui-block posts-full-div '>
      {allPosts.map((item, i) => {
        ...
        const userHasReacted = allMainPostReactions && allMainPostReactions.some(
          (reaction) => reaction.post_id === item._id
        );
          ...
          <article className='hentry post video  ' key={i}>
            <div className='post__author author vcard inline-items'>
              ...
              <div className='author-date'>
                ...
                <h6>
                  {
                    item.page_id != null ?
                      <Link
                        ...
                        href={`pagePublicView/${item.page_id?.page_user_name}`}>
                ...
                {
                  item.post_type == "profile_picture" ? <span className="mx-1 fw-400">
                    has changed profile picture
                  </span > : item.post_type == "cover_picture" ? <span className="mx-1 fw-400">
                    has changed cover picture
                  </span > : ""
                }

                {item.post_type == "Shared" ? (
                  <span className='mx-1 fw-400'>
                    has shared a post
                  </span>
                ) : ""}
                <div className="author-date-custom-margin">
                  ...
                  <div key={item._id} className="mx-2 " class={item.user_id._id !== userId ? "post-privacy-trigger" : ""}>
                    {/* Group post privacy */}
...

            {
              (item.post_background_color == "null" || item.post_background_color == null) ?
                ...
                <>
                  <div className={`color-box ${item.post_background_color}`}
                  style={{
                    backgroundColor: `#${item.post_background_color}`
                  }}
                  >
                    <p dangerouslySetInnerHTML={{ __html: item.description }} />
                    {/* <p dangerouslySetInnerHTML={renderStringWithLink(item.description)} /> */}
                  ...
                  {item.media.map((imageItem, imageIndex) => {
                    if (isImage(imageItem.media)) {
                      ...
                      return (
                        ...
                        <div className={`${item.media.length > 1 ? 'post-multiple-image-video-item' : item.media.length === 1 ? 'one-item' : ''
                          }`}>
                          {/* <Image
                                                                src={`${host}/uploads/posts/${imageItem.media}`}
                                                                width="400"
                      ...
                      {item?.shareMedia?.map((imageItem, imageIndex) => {
                        if (isImage(imageItem.media)) {
                          ...
                          return (
                            ...
                            <div
                              className={`${item?.shareMedia?.length > 1
                ...
                <div className='post-img-auth-img-div'>
                  <div className='id-img-name-div'>
                    ...
                    <div className='modal-min-text-div'>
                      <h5>{item?.share_post_id?.user_id?.first_name}{" "}{item?.share_post_id?.user_id?.last_name}</h5>
            ...
            <div
              ...
              <div className='d-flex justify-content-between align-items-center pb-1 my-0 comment-zone-div' style={{ cursor: "pointer" }}>
                <div className='cursor-pointer  d-flex align-items-center' onMouseEnter={() => { setIsHovered(true); }} onMouseLeave={() => setIsHovered(false)}>
                  {userHasReacted ? (
                    ...
                    <div
                      className='mx-1'
                      onClick={() => handleReactionSelect("like", item._id)}>
                      Like
                    </div>
                  )}
                </div>
                ...
                <div className=''>
                  ...
                  <ul className={`dropdown-menu abt-drops ${isDropdownOpenArray[i] ? 'show' : ''}`} onMouseLeave={() => handleDropdownToggle(i)}>
                    <li>
                      <div className='share-popup' onClick={() => {
                    ...
                    <div className='share-modal-full-div'>
                      ...
                      <div className='id-img-name-full-div'>
                        ...
                        <div className='pst-img-full-div'>
                                ...
                                {sharePost?.media?.map((imageItem, imageIndex) => {
                                  if (isImage(imageItem.media)) {
                                    ...
                                    return (
                                      ...
                                      <div
                                        className={`${sharePost?.media?.length > 1
                                          ? "post-share-multiple-image-video-item"
                          ...
                          <div className='post-img-auth-img-div'>
                            <div className='id-img-name-div'>
                              ...
                              <div className='modal-min-text-div'>
                                <h5>{sharePost?.user_id?.first_name} {sharePost?.user_id?.last_name}</h5>
                        ...
                        <div className='post-modal-btn-div'>
                          ...
                          }} className='post-modal-btn'>Share Post</button>
                        </div>
                      </div>
                    </div>
                  </Box>
            ...
            <div className='comment-section py-2  '>
              <div className={`comment-section-wrapper bg-white ${item.comments.length > 1
                ? "comment-scrollbar"
...

                {
                  item.comments.map((commentitem, commentIndex) => {
                    return (
                      <div className='main-comment-wrapper  py-1 px-2 border  ' scrollstyleReply style={{ backgroundColor: '#f1f1f1' }}>
                        ...
                        <div className='main-comment-text'>
                          ...
                          {/* if someone wants edit main comment */}
                          {
                            ...
                            <div className="">
                              <textarea id="scrollstyle"
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' && !e.shiftKey) {
                                    ...
                                    axiosInstance
                                      .post(`api/update-comments-by-direct-group-post/${editMainCommentPostId}/${editMainCommentId}`, formData, {
                                        headers: {
                                          "Content-Type": "multipart/form-data",
                                        },
                                      })
                              ...
                              <span href="" className="float-right" style={{ color: 'red', cursor: 'pointer' }} onClick={() => { setEditMainCommentStatus(false), setEditMainCommentId(null), setEditMainCommentPostId(null) }}>Cancel</span>
                          ...
                          <div className='main-comment-reaction-emoji-share-wrapper p-1 ' style={{ cursor: 'pointer' }}>
                            <div className='reaction-comment-container '
                              onMouseEnter={() => { setIsCommentReactionHover(true); setIsCommentReactionHoverId(commentitem._id) }} onMouseLeave={() => { setIsCommentReactionHover(false), setIsCommentReactionHoverId(null) }}
                            >
                              {/* comment reaction logics of a user if this user has reacted or not */}

                              {
                                commentitem?.comment_reactions?.filter(reaction => reaction.user_id === userId && reaction.comment_id === commentitem._id)?.map(filteredReaction => (
                                  <div key={filteredReaction._id} style={{ marginTop: '-5px' }} >
                                    <span className="" style={{ color: 'red', fontSize: "12px" }}>{filteredReaction.reaction_type} </span>
                                  </div>
                                ))
                              }

                              {/* Render default emoji if no reactions */}
                              {commentitem?.comment_reactions?.filter(reaction => reaction.user_id === userId && reaction.comment_id === commentitem._id)
                                .length === 0 && (
                            ...
                            {/* comment reaction hover and shows the reactions */}
                            {
                              ...
                              <div
                                onMouseEnter={() => { setIsCommentReactionHover(true); setIsCommentReactionHoverId(commentitem._id) }} onMouseLeave={() => { setIsCommentReactionHover(false); }}

                                className="reaction-comment-icons-wrapper ">
                                <figure
                                  className='reaction-comment-icons mt-1'>
                                  <img
                                    src={`${host}/assets/reactions/like.gif`}
                                    alt='Like emoji'
                                    onClick={(e) => {
                            ...
                            {
                              userId == commentitem.user_id._id && <div className="" onClick={() => { setEditMainCommentStatus(true); setEditMainCommentId(commentitem._id), setEditMainCommentPostId(item._id) }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square ml-2 " viewBox="0 0 16 16">
                              ...
                              {commentitem.user_id._id === userId || item.user_id._id === userId ? (
                                <svg
                                  ...
                                  onClick={() => {
                                    ...
                                    axiosInstance
                                      .post(
                                        "/api/delete-single-group-post-comment",
                                        formData
                                      )
                                      .then((res) => {
                                        allPostss();
                            ...
                            <div className="comment-icon-lists-btn d-flex " onClick={(e) => openWhoesReactOnCommentModal(e, item._id, commentitem._id, "")}>
                              ...
                              {
                                commentitem?.comment_reactions?.length > 0 && <div className="mt-1 text-muted">
...

                          {
                            ...
                            <a className={`${viewMoreRepliesLogicCommentId === commentitem._id && viewMoreRepliesLogicPostId === item._id ? "d-none" : ""}`}
                              data-toggle="collapse"
                              href={`#collapseExample${commentitem._id}`}
                              role="button"
                              aria-expanded="false"
                              aria-controls={`collapseExample${commentitem._id}`}
                              onClick={() => { setViewMoreRepliesLogicCommentId(commentitem._id); setViewMoreRepliesLogicPostId(item._id) }}
                            >
                          ...
                          <div class="collapse" id={`collapseExample${commentitem._id}`}>
                            {
                              commentitem.replies.length > 0 && commentitem.replies.map((replyComment, replyIndex) => {
                                console.log("replyComment__", replyComment)
                                return (
                                  <div className='main-comment-reply-div-wrapper border rounded-right py-1 px-2'
                                    ...
                                    <div className='main-comment-text rounded'>
                                      ...
                                      {/* if someone wants edit  reply comment */}
                                      {
                                        ...
                                        <>
                                          ...
                                          <div className="">
                                            <textarea id="scrollstyle"
                                              ...
                                              onKeyDown={(e) => {
                                                if (e.key === 'Enter' && !e.shiftKey) {
                                                  ...
                                                  axiosInstance
                                                    .post(`api/update-comments-by-direct-group-post/${editMainCommentPostId}/${editMainCommentReplyId}`, editedData)
                                                    .then((res) => {
                                                      if (res.data.status == 200) {
                                                        setRenderComments(res.data);
                                                        setEditMainCommentPostId(null)
                                            ...
                                            <span href="" className="float-right " style={{ color: 'red', cursor: 'pointer' }} onClick={() => { setEditMainCommentReplyStatus(false), setEditMainCommentReplyId(null), setEditMainCommentPostId(null); setEditReplyCommentImageOrVideo(null); setEditReplyCommentImageOrVideoFile(null) }}>Cancel</span>
                                          </div>
                                      ...
                                      <div className='main-comment-reaction-emoji-share-wrapper py-2'>
                                        <div className='reaction-comment-container '
                                          onMouseEnter={() => { setIsReplyCommentReactionHover(true); setIsReplyCommentReactionHoverId(replyComment._id) }} onMouseLeave={() => { setIsReplyCommentReactionHover(false), setIsReplyCommentReactionHoverId(null) }}
                                        >

                                          {/* comment reply reaction logics of a user if this user has reacted or not */}

                                          {
                                            replyComment?.replies_comment_reactions?.filter(reaction => reaction.user_id === userId && reaction.comment_replies_id === replyComment._id)
                                        ...
                                        {/* comment reaction hover and shows the reactions */}
                                        {
                                          isReplyCommentReactionHover && isReplyCommentReactionHoverId == replyComment._id &&
                                          <div
                                            onMouseEnter={() => { setIsReplyCommentReactionHover(true); setIsReplyCommentReactionHoverId(replyComment._id) }} onMouseLeave={() => { setIsReplyCommentReactionHoverId(false); }}
                                            ...
                                            <figure
                                              ...
                                              <img
                                                ...
                                                alt='Like emoji'
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "like");
                                                }}
                                              />
                                              <img
                                                src={`${host}/assets/reactions/love.gif`}
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "love");
                                                }}
                                                alt='Love emoji'
                                              />
                                              <img
                                                src={`${host}/assets/reactions/haha.gif`}
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "haha");
                                                }}
                                                alt='Haha emoji'
                                              />
                                              <img
                                                src={`${host}/assets/reactions/wow.gif`}
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "wow");
                                                }}
                                                alt='Wow emoji'
                                              />
                                              <img
                                                src={`${host}/assets/reactions/sad.gif`}
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "sad");
                                                }}
                                                alt='Sad emoji'
                                              />
                                              <img
                                                src={`${host}/assets/reactions/angry.gif`}
                                                onClick={(e) => {
                                                  handleCommentReactionSubmit(e, item._id, commentitem._id, replyComment._id, "angry");
                                                }}
                                                alt='Angry emoji'
                                              />
                                            </figure>
                                        ...
                                        {replyComment.replies_user_id._id == userId &&
                                          <div className="" onClick={() => { setEditMainCommentReplyStatus(true); setEditMainCommentReplyId(replyComment._id), setEditMainCommentPostId(item._id); }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square ml-2" viewBox="0 0 16 16">
                                              <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                              <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                            </svg>
                                        ...
                                        <div className=''>
                                          {replyComment
                                            .replies_user_id._id ==
                                            userId || item.user_id._id === userId ? (
                                            <svg
                                              xmlns='http://www.w3.org/2000/svg'
                                              width='16'
                                              height='16'
                                              ...
                                              onClick={() => {
                                                ...
                                                axiosInstance
                                                  .post(
                                                    "/api/delete-single-reply-group-post-comment",
                                                    formData
                                                  )
                                                  .then((res) => {
                                                    allPostss();
                                        ...
                                        <div className="comment-icon-lists-btn d-flex " onClick={(e) => openWhoesReactOnCommentModal(e, item._id, commentitem._id, replyComment._id)}>
                                          {[...new Set(replyComment?.replies_comment_reactions?.map(reaction => reaction?.reaction_type))]
                                            .slice(0, 2)
                                            .map((uniqueReaction, index) => (
                                              <div key={index} className="">
                                                <img
                                          ...
                                          {
                                            replyComment?.replies_comment_reactions?.length > 0 && <div className="mt-1 text-muted">
      ...
      <Modal
        ...
        <form>
          <div className='d-flex align-items-center mt-4 m-1'>
            ...
            <div className='author-thumbs'>
              <img
                src={profileImage ? `${host}/uploads/${profileImage}` : 'https://www.kindpng.com/picc/m/24-248253_user-profile-default-image-png-clipart-png-download.png'}
                alt=''
                className='avatar '
              />
            </div>
      ...
      <WhoesAreReactedOnPost isOpen={modalwhoesReactOnPostIsOpen} onClose={closeWhoesReactOnPostModal} value={value} handleChange={handleChange} reactedUserListsOfDirectPosts={reactedUserListsOfDirectPosts}
      ...
      <WhoesAreReactedOnComment isOpen={modalwhoesReactOnCommentIsOpen} onClose={closeWhoesReactOnCommentModal} value={value} handleChange={handleChange}
...
export default GroupPagePostList;
